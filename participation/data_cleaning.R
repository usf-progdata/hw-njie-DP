...
#~ 7 - .x, =>replace ~ recode(.x, "6" = 1, "5" = 2, "4" = 3, "3" = 4, "2" = 5, "1" = 6),
...
dat_bfi %>%
  mutate(
    gender = recode(gender, "1" = "man", "2" = "women", .missing = "(no response)")
  ) %>%
  select(.id, gender, education)
#step3
dat_bfi %>%
  mutate(
    education = recode(education, "1" = "Some HS", "2" = "HS", "3" = "Some College", 
                       "4" = "College", "5" = "Graduate degree", .missing = "(no response)")
  ) %>%
  select(.id, gender, education)
#step 4
dat_bfi %>%
...
dat_bfi %>%
  ...
  select(A1r, C4r, C5r, E1r, E2r, O2r, O5r)

#assign to save the changes!
